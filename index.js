...
const newFlavorsArray = [];

console.log(copy(originalFlavors, newFlavorsArray));
...
function getAverageWordLength(arr) {
  // Create a variable to sum all words across the entire array
  let totalWords = 0;

  for (let i = 0; i < arr.length; i++) {
    // Get the total words in this flavor
    const wordsInFlavor = arr[i].split(" ").length;

    // Add the total words in this flavor to the overall total
    totalWords += wordsInFlavor;
  }

  // Dividing the totalWords by the items in the array will give us the average
...
function getRandomFlavors(arr1, arr2, arr3, arr4) {
  // create new array combining all arrays into one
  const allFlavors = [...arr1, ...arr2, ...arr3, ...arr4];
  // declare array for new 31 randomly generated flavors
  const randomFlavors = [];
  for (let i = 0; i < 31; i++) {
    const random = Math.floor(Math.random() * allFlavors.length);
    // add randomly generated flavor to randomFlavors
    randomFlavors.push(allFlavors[random]);
    // remove generated flavor from allFlavors to stop Math.random from generating reptitive iterations
    allFlavors.splice(random, 1);
  }
